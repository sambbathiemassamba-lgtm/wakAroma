...
// ── Total global (toutes statuts confondus) ──────────────────────
$stmt_total_global = $pdo->prepare("SELECT COUNT(*) FROM commandes WHERE id_user = ?");
$stmt_total_global->execute([$id_user]);
$total_commandes_global = (int)$stmt_total_global->fetchColumn();

// ── Liste des commandes avec photo du premier produit ────────────
$stmt = $pdo->prepare("
    SELECT
        c.id_commande,
        c.numero_commande,
        ...
        (SELECT i.url_image FROM images i
         ...
         WHERE lc2.id_commande = c.id_commande
         LIMIT 1) AS premiere_image
    FROM commandes c
...
.orders-counter__label {
  font-size: 0.78rem;
  color: var(--color-muted);
  font-weight: 500;
  line-height: 1.3;
}
.orders-counter__icon {
  width: 42px; height: 42px;
  border-radius: 10px;
  background: rgba(31,79,46,0.08);
...
.filter-pill { padding:0.4rem 1rem; border:1.5px solid var(--color-border); border-radius:999px; background:var(--color-bg-card); color:var(--color-muted); font-size:0.78rem; font-weight:600; cursor:pointer; text-decoration:none; transition:all var(--transition); }
...
/* ── Tableau ──────────────────────────────────── */
.orders-table-wrap { background:var(--color-bg-card); border:1px solid var(--color-border); border-radius:var(--radius-card); overflow:hidden; }
.orders-table { width:100%; border-collapse:collapse; }
.orders-table thead th { padding:1rem 1.2rem; font-size:0.72rem; letter-spacing:0.12em; text-transform:uppercase; color:var(--color-muted); font-weight:600; text-align:left; background:var(--color-cream); border-bottom:1px solid var(--color-border); }
...
.order-products { display:flex; align-items:center; gap:0.6rem; }
.order-product-img {
...
.order-product-img img { width:100%; height:100%; object-fit:cover; border-radius:10px; }
.order-product-name { font-weight:500; color:var(--color-text); font-size:0.84rem; max-width:200px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
...
.pag-btn { min-width:40px; height:40px; display:flex; align-items:center; justify-content:center; border:1.5px solid var(--color-border); border-radius:10px; background:var(--color-bg-card); color:var(--color-muted); font-size:0.85rem; font-weight:600; text-decoration:none; transition:all var(--transition); }
...
.fade-up:nth-child(1){animation-delay:0.05s} .fade-up:nth-child(2){animation-delay:0.1s} .fade-up:nth-child(3){animation-delay:0.15s} .fade-up:nth-child(4){animation-delay:0.2s} .fade-up:nth-child(5){animation-delay:0.25s}
  ...
  <aside class="boutique-sidebar" aria-label="Navigation espace client">
    ...
    <a href="commandes.php" class="sidebar-nav__item sidebar-nav__item--active">
      ...
      <?php if($total_commandes_global > 0): ?>
        <span class="sidebar-nav__pill"><?= $total_commandes_global ?></span>
      <?php endif; ?>
    </a>
  ...
  <main class="boutique-main">
    ...
    <div class="page-header fade-up">
      <div>
        ...
        <p class="page-header__sub">Retrouvez l'historique complet de vos achats</p>
      </div>
    ...
    <div class="orders-counter fade-up">
      <div class="orders-counter__icon">📦</div>
      <div class="orders-counter__number"><?= $total_commandes_global ?></div>
      <div class="orders-counter__label">
        commande<?= $total_commandes_global > 1 ? 's' : '' ?> passée<?= $total_commandes_global > 1 ? 's' : '' ?><br>
        <span style="color:var(--color-gold); font-weight:600;">depuis votre inscription</span>
      </div>
    </div>
    ...
    <div class="filters-row fade-up">
      ...
      ?>
        ...
        <?php if($filtre_statut === 'tous' && $val === 'tous'): ?>
          <span style="font-size:0.72rem;color:var(--color-muted);margin-left:0.25rem">(<?= $total_rows ?>)</span>
        <?php endif; ?>
      <?php endforeach; ?>
    </div>
      ...
      <table class="orders-table" aria-label="Liste de mes commandes">
        ...
        <tbody>
          ...
          <tr>
              ...
              <div class="order-products">
                <div class="order-product-img">
                  <?php if(!empty($cmd->premiere_image)): ?>
                    <img src="<?= htmlspecialchars($cmd->premiere_image) ?>" alt="<?= htmlspecialchars($premier) ?>">
                  <?php else: ?>
                    🛍️
                  <?php endif; ?>
                </div>
                <div>
                  ...
                  <div class="order-product-qty">
                    ...
                    <?php if(count($noms) > 1): ?>
                      <span style="color:var(--color-gold)"> +<?= count($noms)-1 ?> autre<?= count($noms)-1 > 1 ? 's' : '' ?></span>
                    <?php endif; ?>
                  </div>
                </div>
              </div>
            </td>
            <td data-label="Date" style="color:var(--color-muted);font-size:0.82rem;white-space:nowrap">
            ...
            <td data-label="Total">
              <span class="order-price"><?= number_format((float)$cmd->total, 2, ',', ' ') ?> €</span>
            </td>
          </tr>
          <?php endforeach; ?>
        </tbody>
